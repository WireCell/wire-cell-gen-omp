...
std::vector<double> GenOpenMP::GausDesc::sample(double start, double step, int nsamples) const
{
...
std::vector<double> GenOpenMP::GausDesc::binint(double start, double step, int nbins) const
{
...
std::vector<double> GenOpenMP::GausDesc::weight(double start, double step, int nbins, std::vector<double> pvec) const
{
...
// std::pair<int,int> GenOpenMP::GausDesc::subsample_range(int nsamples, double xmin, double xmax, double nsigma) const
// {
...
GenOpenMP::GaussianDiffusion::GaussianDiffusion(const IDepo::pointer& depo,
...
void GenOpenMP::GaussianDiffusion::set_sampling(const Binning& tbin, // overall time tick binning
...
void GenOpenMP::GaussianDiffusion::set_sampling(
                                                ...
                                                unsigned int weightstrat)
{
    ...
    if (!ntss) {
        cerr << "GenOpenMP::GaussianDiffusion: no time bins for [" << tval_range.first/units::us << "," << tval_range.second/units::us << "] us\n";
        return;
    }
...
*/
void GenOpenMP::GaussianDiffusion::set_sampling_pre(const int diff_idx,
                                                    int & patch_size ,
                                                    double* p_vecs,
                                                    double* t_vecs,
                                                    double* charges, 
...
{
    if (m_patch.size() > 0) {
        cerr << "GenOpenMP::GaussianDiffusion: patch size= 0 , diff number:" << diff_idx << "\n";
...
void GenOpenMP::GaussianDiffusion::clear_sampling(){
  m_patch.resize(0,0); 
  m_qweights.clear();
...
// patch(row,col)
const GenOpenMP::GaussianDiffusion::patch_t& GenOpenMP::GaussianDiffusion::patch() const
{
    return m_patch;
}

const std::vector<double> GenOpenMP::GaussianDiffusion::weights() const
{
    return m_qweights;
}
